...
// Define custom macros to avoid depending on esp_check.h (for maximum core compatibility)
#ifndef ESP_RETURN_ON_ERROR
#define ESP_RETURN_ON_ERROR(x, tag, log) do { \
    esp_err_t _err = (x); \
    if (_err != ESP_OK) { \
        ESP_LOGE(tag, "%s", log); \
        return _err; \
...
#define ESP_RETURN_ON_FALSE(x, err_code, tag, log) do { \
    if (!(x)) { \
        ESP_LOGE(tag, "%s", log); \
        return err_code; \
    } \
...
static const char *TAG = "ES8311_Wire";

// Reg read/write using Wire
static inline esp_err_t es8311_write_reg(uint8_t reg_addr, uint8_t data)
{
    Wire.beginTransmission((uint8_t)ES8311_ADDRESS_0);
    Wire.write(reg_addr);
    Wire.write(data);
...
static inline esp_err_t es8311_read_reg(uint8_t reg_addr, uint8_t *reg_value)
{
    ...
    if (Wire.requestFrom((uint8_t)ES8311_ADDRESS_0, (uint8_t)1) == 1) {
        *reg_value = Wire.read();
        return ESP_OK;
    }
    return ESP_FAIL;
...
static esp_err_t es8311_clock_config(const es8311_clock_config_t *const clk_cfg, es8311_resolution_t res)
{
    ...
    return es8311_sample_frequency_config(nullptr, mclk_hz, clk_cfg->sample_frequency);
}
...
static esp_err_t es8311_fmt_config(const es8311_resolution_t res_in, const es8311_resolution_t res_out)
{
    ...
    reg00 &= 0xBF;
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_RESET_REG00, reg00), TAG, "I2C read/write error");
...
esp_err_t es8311_microphone_config(i2c_master_dev_handle_t dev, bool digital_mic)
{
    ...
    es8311_write_reg(ES8311_ADC_REG17, 0xA8);
...
esp_err_t es8311_init(i2c_master_dev_handle_t dev, const es8311_clock_config_t *const clk_cfg, const es8311_resolution_t res_in, const es8311_resolution_t res_out)
{
    ...
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_RESET_REG00, 0x1F), TAG, "I2C write error");
    delay(20);
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_RESET_REG00, 0x00), TAG, "I2C write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_RESET_REG00, 0x80), TAG, "I2C write error");
    ...
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_SYSTEM_REG0D, 0x01), TAG, "I2C write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_SYSTEM_REG0E, 0x02), TAG, "I2C write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_SYSTEM_REG12, 0x00), TAG, "I2C write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_SYSTEM_REG13, 0x10), TAG, "I2C write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_ADC_REG1C, 0x6A), TAG, "I2C write error");
    ESP_RETURN_ON_ERROR(es8311_write_reg(ES8311_DAC_REG37, 0x08), TAG, "I2C write error");
...
esp_err_t es8311_voice_volume_get(i2c_master_dev_handle_t dev, int *volume)
{
    (void)dev;
    uint8_t reg32;
    ESP_RETURN_ON_ERROR(es8311_read_reg(ES8311_DAC_REG32, &reg32), TAG, "I2C read error");
...
esp_err_t es8311_voice_mute(i2c_master_dev_handle_t dev, bool mute)
{
    (void)dev;
    uint8_t reg31;
    ESP_RETURN_ON_ERROR(es8311_read_reg(ES8311_DAC_REG31, &reg31), TAG, "I2C read error");
...
esp_err_t es8311_voice_fade(i2c_master_dev_handle_t dev, const es8311_fade_t fade)
{
    (void)dev;
    uint8_t reg37;
    ESP_RETURN_ON_ERROR(es8311_read_reg(ES8311_DAC_REG37, &reg37), TAG, "I2C read error");
    reg37 &= 0x0F;
    reg37 |= (fade << 4);
...
esp_err_t es8311_microphone_fade(i2c_master_dev_handle_t dev, const es8311_fade_t fade)
{
    (void)dev;
    uint8_t reg15;
    ESP_RETURN_ON_ERROR(es8311_read_reg(ES8311_ADC_REG15, &reg15), TAG, "I2C read error");
    reg15 &= 0x0F;
    reg15 |= (fade << 4);
...
void es8311_register_dump(i2c_master_dev_handle_t dev)
{
    ...
    for (int reg = 0; reg < 0x4A; reg++) {
        uint8_t value;
        if (es8311_read_reg(reg, &value) == ESP_OK) {
            printf("REG:%02x: %02x\n", reg, value);
        }
    }
}
...
es8311_handle_t es8311_create(int port, const uint16_t dev_addr)
{
    (void)port;
    (void)dev_addr;
    return (es8311_handle_t)1; // dummy return
}
